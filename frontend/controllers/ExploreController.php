...
Yii::setAlias('opus', '@web/uploads/home/opus/');
...
class ExploreController extends BaseController
{
    ...
    {
        $query = new Query;
        $query->select('e.id, title, content, e.created_at, u.username, u.avatar, e.tags')
            ->from('{{%home_post}} as e')
    ...
    public function actionOpuses()
    {
        $query = new Query;
        $query->select('a.id, a.name, p.path, u.username, u.avatar')
            ->from('{{%home_album}} as a')
            ->join('LEFT JOIN','{{%home_opus}} as p', 'p.album_id=a.id')
            ->join('LEFT JOIN','{{%user}} as u', 'a.created_by=u.id')
            ->where('a.status=:type', [':type' => Album::TYPE_PUBLIC])
            ->orderBy('a.id DESC');
        $opusesResult = Yii::$app->tools->Pagination($query, 25);

        return $this->render('opuses', [
            'opuses' => $opusesResult['result'],
            'pages' => $opusesResult['pages'],
        ]);
    }
